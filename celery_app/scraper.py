...
from celery_app.data_processing import connect_to_pinecone, store_data_in_pinecone
...
def get_data_from_url_with_llm(url: str) -> dict:
    ...
    prompt = f"""
        ...
        """
    model = ChatOpenAI(model="gpt-4o", temperature=0, api_key=settings.openai_api_key).bind(
...
def ptt_scrape(kanban: str):
    ...
    index, embeddings = connect_to_pinecone()
    for url in urls:
        try:
        ...
        try:
            store_data_in_pinecone(data, index, embeddings)
        except Exception as e:
            insert_log_into_db(
